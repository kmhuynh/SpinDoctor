function setup=setup_1axon_custom(rcell,kcell,blist,smalldelta,bigdelta,bvecs,tempdir)
...
setup.geometry.rmin = rcell;                                % Minimum radius
setup.geometry.rmax = rcell;                                % Maximum radius
setup.geometry.dmin = 0;                              % Minimum distance between cells (times mean(rmin,rmax))
setup.geometry.dmax = 0;                              % Maximum distance between cells (times mean(rmin,rmax))
...
setup.geometry.in_ratio = 0;                          % Ratio Rin/R, within range [0,0.99]
...
setup.pde.initial_density_out = 0.0;                    % Initial density in OUT
setup.pde.initial_density_ecs = 0.0;                    % Initial density in ECS
setup.pde.permeability_in_out = kcell;                   % Permeability IN-OUT interface
setup.pde.permeability_out_ecs = kcell;                  % Permeability OUT-ECS interface
...
%% Gradient sequences
setup.gradient.values = blist;                    % g-, q-, or b-values [1 x namplitude]
setup.gradient.values_type = "b";                       % Type of values: "g", "q", or "b" or "g"
setup.gradient.sequences = {                            % Gradient sequences {1 x nsequence}
    PGSE(smalldelta, bigdelta)
}';
setup.gradient.directions = bvecs;            % Gradient directions [3 x ndirection]
